...
# Features: MongoDB storage, cooldowns, spam prevention, inline buttons, battle system
...
def get_user(user_id):
    ...
    if not user:
        user = {"_id": user_id, "coins": 0, "cards": [], "last_draw": None, "last_daily": None}
...
async def draw(_, message: Message):
    ...
    card = random.choice(["🔥 Fire Blast", "❄️ Ice Shield", "⚡ Thunder Blade", "🌪 Wind Fury"])
...
async def use(_, message: Message):
    ...
    await message.reply(f"⚔️ {message.from_user.first_name} used {card} on {message.reply_to_message.from_user.first_name} and won the duel!\n🏅 +{WIN_REWARD} coins! 🎉")
...
async def shop(_, message: Message):
    buttons = [[InlineKeyboardButton("Buy Shield - 3 Coins", callback_data="buy_shield")],
               [InlineKeyboardButton("Buy Scroll - 5 Coins", callback_data="buy_scroll")]]
    await message.reply("🛍 Shop: Use your coins to buy powers:", reply_markup=InlineKeyboardMarkup(buttons))
...
async def handle_shop(client, callback):
    ...
    if data == "buy_shield" and user["coins"] >= 3:
        ...
        add_card(user_id, "🛡 Shield")
        await callback.answer("Bought Shield!")
    elif data == "buy_scroll" and user["coins"] >= 5:
        reward_coins(user_id, -5)
        add_card(user_id, "📜 Scroll")
        await callback.answer("Bought Scroll!")
    else:
        await callback.answer("Not enough coins!", show_alert=True)

# === INLINE QUERY SUPPORT (Optional Search Cards) ===
...
# === UNKNOWN COMMANDS ===
@client.on_message(filters.text & ~filters.command(["start", "join", "draw", "use", "inventory", "coins", "daily", "shop", "skip", "end"]))
async def unknown(_, message: Message):
    pass  # Don't reply to random texts to avoid spam
